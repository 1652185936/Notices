...
## 6. ⚠️ 最大短板 = UI 视觉还原（接手第一优先级）

功能能跑，但**界面不像华为**。用户实测后明确不满意。已知具体差距（对照用户提供的华为真机截图）：

### 无界笔记工具栏（ui/canvas/CanvasScreen.kt 里的 `CanvasToolbar`/`ToolButton`/`BrushPanel`）
- 现在用的是**通用 Material 图标**（手形、虚线方块、橡皮是把"扫帚"`CleaningServices`）——华为是**精致的笔尖/橡皮专用图标**，需要自绘 vector 或换更贴近的图标。
- 选中的笔做成了**很重的蓝色实心圆**——华为是轻微高亮、笔"抬起"。
- 华为工具栏**右侧内嵌了线型预设(几档波浪/粗细) + 快捷色点**；当前只有一个色点 + 缩放，右侧空。
- 工具栏底色：华为是**淡蓝灰**，当前是纯白发飘。

### 首页列表（ui/notes/NotesListScreen.kt）
- 当前是**光秃秃的描边卡片 + 通用底部导航**；华为首页是更丰富的卡片网格、留白与层次更好。

### 其它
- 分页编辑器工具栏、各页空状态/动效/触觉反馈、深色画布适配都还粗糙。
- 套索的"调整大小/转文本"、橡皮"擦除整个笔划"开关面板等华为细节未做。

**接手建议**：拿用户的华为截图，**一个界面一个界面对着抠**（不要大包大揽）。优先级：①无界笔记工具栏 ②首页列表 ③分页编辑器工具栏。`docs/10-差距分析与缺陷清单.md` 有完整缺陷/待办清单（E=已完成，F=待办）。
